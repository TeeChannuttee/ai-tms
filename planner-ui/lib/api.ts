...
function getAuthToken(): string | null {
    if (typeof window !== 'undefined') {
        ...
        // console.log("[DEBUG] getAuthToken called. Token exists?", !!token);
        return token;
    }
    return null;
}
...
async function fetchWithAuth(url: string, options: RequestInit = {}) {
    ...
    if (token) {
        ...
        console.warn(`[DEBUG] No auth token found. Request to ${url} might fail.`);
    }
    ...
    if (!response.ok) {
        if (response.status === 401) {
            console.error(`[DEBUG] 401 Unauthorized for ${url}. Token sent: ${!!token}`);
            // Optional: Redirect to login or clear token if invalid
...
export const fleetAPI = {
    ...
    getVehicles: async (params?: { status?: string }): Promise<Vehicle[]> => {
        const queryParams = new URLSearchParams(params as any).toString();
        return fetchWithAuth(`/fleet/vehicles${queryParams ? `?${queryParams}` : ''}`);
    },
...
export const routesAPI = {
    ...
    addStopToRoute: async (routeId: string, orderId: string): Promise<any> => {
        return fetchWithAuth(`/routes/${routeId}/stops`, {
            method: 'POST',
            body: JSON.stringify({ order_id: orderId }),
        });
    },

    removeStop: async (routeId: string, stopId: string): Promise<any> => {
        return fetchWithAuth(`/routes/${routeId}/stops/${stopId}`, {
            method: 'DELETE',
        });
    },

    createRoute: async (routeData: any): Promise<any> => {
        return fetchWithAuth('/routes', {
            method: 'POST',
...
export const analyticsAPI = {
    ...
    getWeeklyReport: async (week?: number): Promise<any> => {
        ...
        const response = await fetch(`${API_BASE_URL}/analytics/reports/weekly${query}`, { headers });
...
export const modelMonitoringAPI = {
    getMetrics: async (modelName: string, days: number = 7): Promise<any[]> => {
        return fetchWithAuth(`/models/${modelName}/metrics?days=${days}`);
    },

    checkDrift: async (modelName: string): Promise<any> => {
        return fetchWithAuth(`/models/${modelName}/drift`);
    },

    triggerDailyCalculation: async (modelName: string, date?: string): Promise<any> => {
        const queryParams = date ? `?date=${date}` : '';
        return fetchWithAuth(`/models/${modelName}/calculate-daily${queryParams}`, {
            method: 'POST'
        });
    },
};
